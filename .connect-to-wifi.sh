...
wifi_name=$(iwctl station wlan0 scan > /dev/null; iwctl station wlan0 get-networks |
	...
	sort --general-numeric-sort --ignore-case |
	fzf --reverse --bind 'load:reload:iwctl station wlan0 scan > /dev/null; iwctl station wlan0 get-networks | tail -n +5 | sed "s/\x1b\[\(0m\|1;90m\**\)//g" | sed "s/^......//" | grep -Pv "^$" | sort --general-numeric-sort --ignore-case; sleep 0.3' |
