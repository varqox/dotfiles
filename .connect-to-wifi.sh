...
wifi_name=$(iwctl station wlan0 scan > /dev/null; iwctl station wlan0 get-networks |
	...
	fzf --reverse --no-sort --bind 'load:reload-sync:sleep 0.3; iwctl station wlan0 scan > /dev/null; iwctl station wlan0 get-networks | tail -n +5 | sed "s/\x1b\[\(0m\|1;90m\**\)//g" | sed "s/^......//" | grep -Pv "^$"' |
